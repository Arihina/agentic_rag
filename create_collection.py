"""
Индексирует результаты парсинга MinerU в OpenSearch.
...
Чанки из обоих источников попадают в один индекс;
...
Использование:
    ...
    python create_collection_opensearch.py \
...
def build_chunks(
    ...
    # имя физического файла-источника (для трассируемости)
    source_filename: str,
...
def _bulk_index_chunks(
    ...
    success_count, errors = bulk(client, actions, raise_on_error=True)
...
def index_documents(
    ...
    client = get_client()
    create_index(client, index_name=index_name, recreate=reset)
    ...
    print(f"  Итого       : {total} в индексе '{index_name}'")


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Индексация MinerU (JSON + MD) → OpenSearch")
    ...
    parser.add_argument("--reset",         action="store_true",
                        help="Удалить индекс перед индексацией")
